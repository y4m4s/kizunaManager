...
export function clampLevel(level: number): number {
  return Math.max(1, Math.min(MAX_BOND_LEVEL, Number(level) || 1))
}
...
export function clampCurrentExp(level: number, currentExp: number): number {
  ...
  return Math.max(0, Math.min(Number(currentExp) || 0, required - 1))
}
...
export function cumulativeExpToLevel(level: number): number {
  ...
  const cached = cumulativeCache.get(normalized)
  if (cached !== undefined) {
    return cached
  }
  let total = 0
  ...
  cumulativeCache.set(normalized, total)
  return total
}
...
export function progressRatio(
  ...
  const currentTotal = cumulativeExpToLevel(current) + clampCurrentExp(current, currentExp)
  const targetTotal = cumulativeExpToLevel(target)
  if (targetTotal <= 0) {
    return 1
  }

  return Math.max(0, Math.min(1, currentTotal / targetTotal))
}

export function projectLevelAfterGain(
  currentLevel: number,
  currentExp: number,
  gainedExp: number,
): [number, number] {
  let level = clampLevel(currentLevel)
  let expInLevel = clampCurrentExp(level, currentExp)
  let remainingGain = Math.max(0, Number(gainedExp) || 0)

  while (remainingGain > 0 && level < MAX_BOND_LEVEL) {
    const required = BOND_EXP_TABLE[level] || 0
    const needed = Math.max(0, required - expInLevel)
    if (needed === 0) {
      level += 1
      expInLevel = 0
      continue
